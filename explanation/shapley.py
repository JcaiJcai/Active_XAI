...
class Explainer_shap(nn.Module):
    def __init__(self, device, MIL_model='ABMIL', subset_num=3, shuffle=True, shuffle_time=2):
        super().__init__()
        self.device = device
    ...
    def shapley_value(self, search_indices, data, label, model):
        ...
        with torch.no_grad():
            ...
            left_logits = []
            subset_data = [left_data[:,i::self.subset_num,:] for i in range(self.subset_num)] # 从第 i 个元素开始，每隔 subset_num 个取一个

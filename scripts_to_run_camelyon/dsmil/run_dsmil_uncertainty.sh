...
for top_k_for_annotation in 20 30 40; do
    for seed in 2021; do
        ...
        # 每个实验的脚本文件路径
        output_script="/u/jcai1/code/usefulxai/scripts/dsmil/${project_name}.sh"

    # 生成脚本文件
    cat > "$output_script" <<EOT
#!/bin/bash
#SBATCH --job-name=${project_name}
#SBATCH --output=/u/jcai1/code/usefulxai/paper_results/dsmil/${project_name}.out
...
--k_fold=3 \
--teacher_init=/u/jcai1/code/usefulxai/paper_results/pure_model/dsmil_0507_1818 \
--cl_alpha=0.1 \
--mm_sche \
--init_stu_type=fc \
--seed=${seed} \

...
for top_k_for_annotation in 20 30 40; do
    for seed in 2021; do
        ...
        attn_alpha=0.01
        project_name="abmil_uncertainty_top${top_k_for_annotation}_${attn_alpha}_${annotation_alpha}_${seed}"
        # 每个实验的脚本文件路径
        output_script="/u/jcai1/code/usefulxai/scripts/abmil/${project_name}.sh"

    # 生成脚本文件
    cat > "$output_script" <<EOT
#!/bin/bash
#SBATCH --job-name=${project_name}
#SBATCH --output=/u/jcai1/code/usefulxai/paper_results/abmil/${project_name}.out
...
--k_fold=3 \
--teacher_init=/u/jcai1/code/usefulxai/paper_results/pure_model/abmil_0507_1818 \
--cl_alpha=0.1 \
--mm_sche \
--init_stu_type=fc \
--seed=${seed} \
